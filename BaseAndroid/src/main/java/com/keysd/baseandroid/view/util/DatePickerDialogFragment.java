...
/**
 * DatePicker Dialog that can be fully configured. See
 * {@link com.keysd.baseandroid.view.util.DatePickerBuilder} class
 *
 * @author "Luis Alberto Regalado Gomez"
...
public class DatePickerDialogFragment extends DialogFragment {
  ...
  public Dialog onCreateDialog(@NonNull Bundle savedInstanceState) {
    if (mAttributes == null) {
      mAttributes = new DatePickerBuilder();
    }
    Calendar initial = Calendar.getInstance();
    initial.setTime(mAttributes.getInitialDate());
    DatePickerDialog datePickerDialog =
        new DatePickerDialog(getActivity(), mAttributes.getOnDateSetListener(),
    ...
    if (mAttributes.getOnCleanDateListener() != null) {
      datePickerDialog
          .setButton(DatePickerDialog.BUTTON_NEGATIVE, getText(mAttributes.getClearButtonText()),
  ...
  private void setCustomPicker(DatePickerDialog datePickerDialog) {
    ...
    if (dpView != null) {
      ...
      if (llFirst != null) {
        ...
        if (mAttributes.getHeaderLayout() <= 0) {
          return;
        }
        ...
        dpView.addView(v);
        if (mAttributes.getHeaderHeight() <= 0) {
          return;
        }
