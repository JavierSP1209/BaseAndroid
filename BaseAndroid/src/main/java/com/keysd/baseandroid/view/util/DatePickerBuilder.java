 ...
 * {@link com.keysd.baseandroid.view.util.DatePickerDialogFragment}. Several attributes can be
 * set in order to
...
public class DatePickerBuilder implements Serializable {
  ...
  /**
   * If defined, a Negative Button is added to the DatePicker and, when clicked, the action
   * defined by the
   * listener is executed.
   */
  private DialogInterface.OnClickListener mOnCleanDateListener;
  /**
   * The initial date that the picker will display. If null, {@link com.keysd.baseandroid.util
  ...
  public DatePickerBuilder setOnDateSetListener(
  ...
  public DatePickerBuilder setOnCleanDateListener(
      DialogInterface.OnClickListener mOnCleanDateListener) {
   ...
   *
   * @param initialDate
   * 	A {@link java.util.Date} object with the date to display initially. If null, it is replaced
   * 	with
   * 	{@link com.keysd.baseandroid.util.KeyDictionary#ZERO_DATE}
   ...
   *
   * @param minDate
   * 	A {@link java.util.Date} object for the maximum date to display. The picker won't display
   * 	any date
   * 	before this one.
   * @return This object to chain the set call
   */
  ...
  public DatePickerBuilder setMaxDate(@Nullable Date maxDate) {
    ...
    //		if (maxDate != null && maxDate.before(mInitialDate)) mInitialDate = maxDate;
   ...
   * 	A String resource with the text to show at the positive button. If this parameter is <= 0,
   * 	it is replaced
  ...
  /**
   * Sets the divider for the date picker numbers. A Shape with a Solid color is recommended for
   * the divider
   *
   * @param divider
  ...
  public DatePickerBuilder setDivider(@DrawableRes int divider) {
    if (divider <= 0) {
      divider = R.drawable.date_picker_default_divider;
    }
   ...
   *
   * @param headerLayout
   * 	The layout that represents the header that will be displayed over the DatePicker. If this
   * 	parameter is
  ...
  /**
   * Sets the title for the header. If a custom header layout is set by using
   * {@link #setHeaderLayout(int)},
  ...
  /**
   * Method to give the option to the user to give a String as the title for the header. If a
   * custom header layout
   * is set by using {@link #setHeaderLayout(int)}, this attribute is ignored.
   * Note that calling this method will make that if a header was set previously with
   * {@link #setHeaderTitle(int)},
   ...
   *
   * @param headerHeight
   * 	A dimen resource for the height. If this parameter is <= 0, it is ignored when building the
   * 	header.
   * @return This object to chain the set call
   */
