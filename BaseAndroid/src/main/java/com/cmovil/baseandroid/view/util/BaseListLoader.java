 ...
 * This loader provides an interface in order to give the client class the ability to tell this loader
 * what to load in the background.
 * This loader is based on managing lists, so it is simple but very flexible given its generic nature.
 *
...
public class BaseListLoader<D> extends AsyncTaskLoader<List<D>> {
	...
	public List<D> loadInBackground() {
		if (mBaseLoaderListener != null) mList = mBaseLoaderListener.onLoadInBackground();
		else mList = new LinkedList<>();
		return mList;
	}
	...
	/**
	 * Interface that serves as a listener for the client user to be able to tell the Loader what to load
	 * in the background
	 *
	 * @param <D>
	...
	public interface BaseLoaderListener<D> {
		 ...
		 */
		public List<? extends BaseLoaderObserver> createLoaderObservers(BaseListLoader<D> baseListLoader);

		/**
		 * Method that carries out the fetching of the data. This is for telling the loader how is the data
		 * fetched
		 *
		 * @return
	...
	/**
	 * Base observer that contains the main observer functionality and lets the subclasses define what and how they
	 * are observing any data source. These objects may be added to {@link BaseListLoader}
	 * class, where the main methods are used. When the client determines that a change needs to be notified, the method
