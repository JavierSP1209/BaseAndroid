...
/**
 * Public abstract class that should be implemented for customize both
 * {@link android.os.AsyncTask#onPreExecute()} and
 * {@link android.os.AsyncTask#onPostExecute(Object)} functions of an async task, this will allow
 * the calling class to
 * manage its results depending on its needs
 *
 * @param <T> Type of parameter used on {@link android.os.AsyncTask#onPostExecute(Object)}
...
public abstract class CustomAsyncTaskEventListener<T> {
   ...
   *
   * @param result Downloaded bitmap, if an error occurs during download or the
