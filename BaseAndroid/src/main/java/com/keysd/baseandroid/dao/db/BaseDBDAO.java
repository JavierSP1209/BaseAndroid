...
import com.keysd.baseandroid.dao.db.helper.BaseDatabaseOpenHelper;
import com.keysd.baseandroid.dao.db.helper.DatabaseDictionary;
import com.keysd.baseandroid.model.db.BaseModel;
...
/**
 * Base data base helper contains all the base functions for common data base operations like,
...
public abstract class BaseDBDAO<T extends BaseModel> {
   ...
   * Constructor
   *
   * @param tableName              Table name in which this queries will be executed
   * @param baseDatabaseOpenHelper data base open helper to be used in this class
   */
  ...
  /**
   * Sets the progress updater to be used on the data base helper, this is used when the data base
   * is initialized
   *
   * @param progressUpdater Listener to be called after a table its initialized
   */
   ...
   *
   * @param insertObject Object to be get the table values
   ...
   * {@link #update(com.keysd.baseandroid.model.db.BaseModel)}
   ...
   *
   * @param tableName     Table name in which this query will be executed
   * @param columns       The columns to include, if null then all are included
   * @param projectionMap The projection map maps from column names that the caller passes into
   *                       query to database column names. This is
   *                      useful for renaming columns as well as disambiguating column names when
   *                      doing joins. For example you could map
   *                      "name" to "people.name". If a projection map is set it must contain all
   *                      column names the user may request,
   *                      even if
   ...
   *
   * @param tableName     Sets the list of tables to query. Multiple tables can be specified to
   *                       perform a join. For example: setTables
   *                      ("foo,
   ...
   */
  protected Cursor query(String tableName, String selection, String[] selectionArgs, String[]
      columns,
      Map<String, String> projectionMap) {
    return query(tableName, selection, selectionArgs, columns, projectionMap, null, null, null,
                 null);
  }
   ...
   *                       clause (excluding the GROUP BY itself).
   *                      Passing null will cause the rows to not be grouped.
   * @param having        A filter declare which row groups to include in the cursor, if row
   *                       grouping is being used, formatted as an SQL
   *                      HAVING clause (excluding the HAVING itself). Passing null will cause all
   *                      row groups to be included, and is
   *                      required when row grouping is not being used.
   * @param sortOrder     How to order the rows, formatted as an SQL ORDER BY clause (excluding
   *                       the ORDER BY itself). Passing null will
   ...
   * @return A Cursor over all rows matching the query
   */
  protected Cursor query(String tableName, String selection, String[] selectionArgs,
      String[] columns,
    ...
    Cursor cursor = builder.query(db, columns, selection, selectionArgs, groupBy, having,
                                  sortOrder, limit);
   ...
   *
   * @param query         Query to execute in the db
   * @param selectionArgs Selection arguments for "?" components in the selection
   * @return A Cursor over all rows matching the query
   */
   ...
   *
   * @param insertObject Object to be get the values to insert
   * @return The id of the inserted row, or -1 if an error occurs while opening the data base
   * @throws com.keysd.baseandroid.dao.db.DBException if something goes wrong during SQL
  ...
  /**
   * Insert a list of object to the data base, this method must be used for performance, in other
	 * cases use the base
   * insert one
   *
   * @param insertObjects List of objects to insert
   * @throws com.keysd.baseandroid.dao.db.DBException if something goes wrong during SQL
   * statements execution
   */
   ...
   * Insert a list of object to the data base, this method must be used for performance, in other
   * cases use the base
   * insert one, this method should only be used when the
	 * {@link android.database.sqlite.SQLiteDatabase} object is
   * managed outside the insert method, in other case use
	 * {@link #insert(com.keysd.baseandroid.model.db.BaseModel)}
   *
   * @param insertObjects List of objects to insert
   * @param db            SQLite data base object to be used
   * @throws com.keysd.baseandroid.dao.db.DBException if something goes wrong during SQL
   * statements execution
   */
  ...
  /**
   * Insert a list of object to the data base, this method should be used prior API 16, due to
	 * SQLite version
   *
   * @param insertObjects List of objects to insert
   * @throws com.keysd.baseandroid.dao.db.DBException if something goes wrong during SQL
  ...
  private void insertV8(List<T> insertObjects, SQLiteDatabase db) throws DBException {
    try {
      ...
      if (insertObjects != null && !insertObjects.isEmpty()) {
        ...
        //Add ? as many insert objects are on the movement list, this can pre-processed because
				// all the
    ...
    try {
      ...
      if (insertObjects != null && !insertObjects.isEmpty()) {
        ...
        if (size > 0) {
          ...
          //Save params string
          params = sql.toString();
          //Get the maximum number of batch inserts that can be used, since the max number of SQL
					// parameters
          // are 999
          ...
          while (iterator.hasNext()) {
            ...
            int insertCount = remainingInserts >= maxInsertCount ? maxInsertCount :
                remainingInserts;
   ...
   *
   * @param sql           Builder to initialize
   * @param initialValues ContentValues to be used on the insert statement
   */
   ...
   * insert one
   *
   * @param insertObjects Object to be get the values to insert
   * @throws com.keysd.baseandroid.dao.db.DBException if something goes wrong during SQL
	 * statements execution
   */
   ...
   *
   * @param id Database id to be deleted from the data base table
   * @return the number of rows affected, 0 otherwise. To remove all rows and get a
   * count pass "1" as the whereClause.
   * @throws DBException if something goes wrong during SQL statements execution
   */
   ...
   * Delete an specific row from the selected table in database
   *
   * @param objectToDelete Object with the fields to search the row to delete, this object could
	 *                        have only the attributes defined on
   *
	 *                       {@link com.keysd.baseandroid.model.db.BaseModel#getPrimaryKeySelectionArgs()}
   * @return the number of rows affected, 0 otherwise. To remove all rows and get a
   * count pass "1" as the whereClause.
   * @throws DBException if something goes wrong during SQL statements execution
   */
   ...
   *
   * @param objectToUpdate Object with the all the values to update
   * @return the number of rows affected, 0 otherwise. To remove all rows and get a
   * count pass "1" as the whereClause.
   * @throws DBException if something goes wrong during SQL statements execution
   */
  public Integer update(T objectToUpdate) throws DBException {
    ...
    try {
      ...
      return db.update(tableName, fillMapValues(objectToUpdate), getPrimaryKeyFilter(),
                       selectionArgs);
    } catch (SQLException e) {
      throw new DBException(e.getMessage(), e);
    } finally {
   ...
   *
   * @param id      Id that will be searched in the database
   * @param columns The columns to include, if null then all are included
   ...
   * @throws DBException if something goes wrong during SQL statements execution
   */
  public Cursor getById(Integer id, String[] columns, Map<String, String> projectionMap) throws
      DBException {
    //Add table name to default filter id string in order to use full table name for join queries
    String[] selectionArgs = new String[]{String.valueOf(id)};
    try {
      return query(getDefaultTableJoin(), getPrimaryKeyFilter(), selectionArgs, columns,
                   projectionMap);
    } catch (SQLException e) {
      throw new DBException(e.getMessage(), e);
    }
   ...
   * Gets all the objects of the selected table with the selected id
   *
   * @param objectToSearch Object with the fields to search the row to delete, this object could
   *                        have only the attributes defined on
   *
	 *                       {@link com.keysd.baseandroid.model.db.BaseModel#getPrimaryKeySelectionArgs()}
   * @param columns        The columns to include, if null then all are included
   ...
   *                       useful for renaming columns as well as disambiguating column names when
   *                       doing joins. For example you could map
   *                       "name" to "people.name". If a projection map is set it must contain
	 *                       all column names the user may request,
   *                       even if
   *                       the key and value are the same.
   * @return Cursor positioned to matching word, or null if not found.
   * @throws DBException if something goes wrong during SQL statements execution
   */
  public Cursor getById(T objectToSearch, String[] columns, Map<String, String> projectionMap)
      throws DBException {
    //Add table name to default filter id string in order to use full table name for join queries
