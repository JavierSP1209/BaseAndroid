 ...
 * Description:
 * <p>
 * Class that will be the connection between database and view classes, so it will include
 * functions for inserting,
 * deleting, updating and getting rows in the data base, but will use DAO objects for getting and
 * returning data
 */

package com.keysd.baseandroidtest.controller;

import android.content.Context;
import android.database.Cursor;
import android.util.Log;
import com.keysd.baseandroid.controller.BaseDBController;
import com.keysd.baseandroid.dao.db.DBException;
import com.keysd.baseandroidtest.dao.db.SampleDAO;
import com.keysd.baseandroidtest.dao.db.helper.DatabaseDictionary;
import com.keysd.baseandroidtest.model.db.State;
import com.keysd.baseandroidtest.util.KeyDictionary;
...
public class SampleController extends BaseDBController<State> {
  ...
  public State fillUpObject(Cursor cursor) {
    ...
    state.setIdServer(cursor.getInt(
        cursor.getColumnIndexOrThrow(DatabaseDictionary.State.COLUMN_NAME_ID_SERVER)));
    state.setName(
        cursor.getString(cursor.getColumnIndexOrThrow(DatabaseDictionary.State.COLUMN_NAME_NAME)));
    return state;
  }
   ...
   * @return A {@link com.keysd.baseandroidtest.model.db.State} object with the values from the
   * database or an empty
   * object if no results where found
   *
   * @throws com.keysd.baseandroid.dao.db.DBException
  ...
  /**
   * Test and timing the batch insert depending on the number of test insertions, this test will
	 * create dummy states
  ...
  public void testJoin() throws DBException {
    ...
    String[] columns = new String[]{DatabaseDictionary.State._ID, "stateName", "stateIdServer",
        "stateAux"};
    HashMap<String, String> projectionMap = new HashMap<String, String>();
    projectionMap
        .put(DatabaseDictionary.State._ID,
             DatabaseDictionary.State.NAME + "." + DatabaseDictionary.State._ID);
    projectionMap
        .put("stateName", DatabaseDictionary.State.COLUMN_FULL_NAME_NAME + " AS stateName");
    projectionMap.put("stateIdServer",
                      DatabaseDictionary.State.COLUMN_FULL_NAME_ID_SERVER + " AS stateIdServer");
    projectionMap.put("stateAux",
                      DatabaseDictionary.StateAux.NAME + "."
                          + DatabaseDictionary.StateAux.COLUMN_NAME_NAME + " AS stateAux");
